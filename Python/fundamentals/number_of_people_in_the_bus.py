...
def number(bus_stops):
    getin = [bus_stops[i][0] for i in range(len(bus_stops))]
    getof = [bus_stops[i][1] for i in range(len(bus_stops))]
    total_passangers = abs(sum(getin)-sum(getof))
...
def number(bus_stops):
    getin = [bus_stops[i][0] for i in range(len(bus_stops))]
    getof = [bus_stops[i][1] for i in range(len(bus_stops))]
    total_passangers = sum(getin)-sum(getof)
    if total_passangers < 0:
        return total_passangers*(-1)
    return total_passangers